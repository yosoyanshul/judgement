<template>
  <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 transition-all duration-500">
    <!-- Step 1: Number of Players -->
    <div v-if="step === 1">
      <h2 class="text-3xl font-bold text-center text-gray-700 dark:text-gray-200 mb-6">
        How many players?
      </h2>
      <div class="flex justify-center items-center space-x-4">
        <input
          ...
          min="2"
          max="10"
          class="text-center text-2xl font-bold w-32 p-3 border-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
      ...
      <div class="text-center mt-6">
        <button
          @click="goToNextStep"
          class="bg-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-700 transition-colors text-xl"
        >
          Next
        </button>
      </div>
      <p v-if="error" class="text-red-500 dark:text-red-400 text-center mt-4">{{ error }}</p>
    </div>
